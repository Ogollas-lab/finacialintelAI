# FinancialIntelAI — Database Schema

> **Database:** Supabase PostgreSQL 15
> **Version:** 1.0 — MVP Schema
> **Date:** June 7, 2026
> **Extensions:** `uuid-ossp`, `pg_cron`, `pg_net`, `pgcrypto`
...
│  │ (extends     │  │              │  │ (future multi-user)      │   │
│  │  auth.users) │  │              │  │                          │   │
│  └──────────────┘  └──────────────┘  └──────────────────────────┘   │
│                                                                      │
│  DATA INTEGRATIONS                                                   │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────────────────┐   │
│  │ connections  │  │ sync_logs    │  │ raw_transactions         │   │
│  └──────────────┘  └──────────────┘  └──────────────────────────┘   │
│                                                                      │
│  FINANCIAL DATA                                                      │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────────────────┐   │
│  │ metric_      │  │ daily_       │  │ industry_                │   │
│  │ snapshots    │  │ financials   │  │ benchmarks               │   │
│  └──────────────┘  └──────────────┘  └──────────────────────────┘   │
│                                                                      │
│  FORECASTING                                                         │
│  ┌──────────────┐  ┌──────────────┐                                 │
...
| Category | Tables | Row Growth Pattern |
|---|---|---|
| Identity & Access | 3 | Slow (user signups) |
| Data Integrations | 3 | Medium (sync logs daily) |
| Financial Data | 3 | High (daily metric snapshots, daily financials) |
| Forecasting | 2 | Medium (daily recalculation) |
| AI & Insights | 2 | Low (weekly generation) |
| Reports | 1 | Low (monthly generation) |
| Billing | 2 | Low (monthly events) |
| Leads | 1 | Medium (free tool usage) |
...
```
                            ...
                            └──────┬──────┘
                                   ...
                                   ▼
 ┌─────────────┐            ┌─────────────┐            ┌────────────────┐
 │subscriptions│◄───────────│  companies  │───────────►│  connections   │
 │             │   1:1      │             │   1:N      │                │
 │ stripe_*    │            │ name        │            │ source_type    │
 │ plan        │            │ industry    │            │ access_token   │
 │ status      │            │ revenue_rng │            │ status         │
 └─────────────┘            └──────┬──────┘            └───────┬────────┘
                                   │                           │ 1:N
                          1:N │    │ 1:N                       ▼
                              │    │              ┌────────────────────┐
                              ...
                              ▼    │              │                    │
                   ┌──────────────┐│              │ status, records,   │
                   │daily_        ││              │ error_message      │
                   │financials    ││              └────────────────────┘
                   │              ││
                   │ date         ││ 1:N
                   │ revenue      │├────────────► ┌────────────────────┐
                   │ expenses     ││              │  metric_snapshots  │
                   │ cash_balance ││              │                    │
                   └──────────────┘│              │ metric_name        │
                                   │              │ metric_value       │
                                   │              │ period_date        │
                   ...
                   │ overrides    │      │  industry_         │
                   │              │      │  benchmarks        │
                   │ event_name   │      │                    │ (Reference table)
                   │ amount       │      │  industry_category │
                   │ event_date   │      │  revenue_range     │
                   └──────────────┘      │  metric_name       │
                                         │  p25, p50, p75     │
                          ┌──────────┐   └────────────────────┘
                          │ reports  │
                          │          │   ┌────────────────────┐
                          │ type     │   │  leads             │ (Free tools)
                          │ file_url │   │                    │
                          │ range    │   │  email             │
                          └──────────┘   │  tool_used         │
                                         │  scores            │
...
> The central entity — every financial data record belongs to a company.
...
> Future multi-user support. MVP: only the owner exists.
...
> OAuth integration connections to external data sources.
...
> Audit trail of every data synchronization attempt.
...
### 3.6 raw_transactions

> Normalized financial transactions from all integrated sources.
...
### 3.7 daily_financials

> Aggregated daily financial summary — derived from raw_transactions.
...
### 3.8 metric_snapshots

> Computed KPI values at different time granularities.
...
### 3.9 industry_benchmarks

> Reference data for industry comparison. Seeded from curated datasets.
...
> Weekly cash flow forecast projections.
...
### 3.11 forecast_overrides

> User-created manual events that modify the forecast.
...
> Generated financial reports (PDF files stored in Supabase Storage).
...
> Stripe subscription state mirrored locally for fast access.
...
> Email leads captured from free tools (Product-Led Growth).
...
```
supabase/migrations/
├── 20260607000001_create_enums.sql
├── 20260607000002_create_profiles.sql
├── 20260607000003_create_companies.sql
├── 20260607000004_create_company_members.sql
├── 20260607000005_create_connections.sql
├── 20260607000006_create_sync_logs.sql
├── 20260607000007_create_raw_transactions.sql
├── 20260607000008_create_daily_financials.sql
├── 20260607000009_create_metric_snapshots.sql
├── 20260607000010_create_industry_benchmarks.sql
├── 20260607000011_create_forecasts.sql
├── 20260607000012_create_forecast_overrides.sql
├── 20260607000013_create_insights.sql
├── 20260607000014_create_reports.sql
├── 20260607000015_create_subscriptions.sql
├── 20260607000016_create_billing_events.sql
├── 20260607000017_create_leads.sql
├── 20260607000018_create_rls_policies.sql
├── 20260607000019_create_indexes.sql
├── 20260607000020_create_functions_triggers.sql
├── 20260607000021_seed_benchmarks.sql
└── 20260607000022_setup_cron_jobs.sql
```
...
```bash
# Local development
supabase db reset                    # Drop & recreate from migrations
...
supabase db remote commit            # Pull remote changes as migration
...
|---|---|---|
| `raw_transactions` | Per plan (12mo / 24mo / ∞) | Monthly cron: delete rows older than plan limit |
| `daily_financials` | Per plan (12mo / 24mo / ∞) | Monthly cron: delete rows older than plan limit |
| `metric_snapshots` | Per plan (12mo / 24mo / ∞) | Monthly cron: delete rows older than plan limit |
| `forecasts` | 90 days of historical forecasts | Weekly cron: delete forecasts older than 90 days |
| `insights` | 12 months | Monthly cron: delete insights older than 12 months |
| `sync_logs` | 90 days | Weekly cron: delete logs older than 90 days |
| `reports` | Per plan (12mo / 24mo / ∞) | Monthly cron: delete reports + Storage files older than plan limit |
| `billing_events` | Indefinite | No cleanup — regulatory compliance |
| `leads` | 24 months | Monthly cron: delete unconverted leads older than 24 months |

---

> *Schema designed for MVP launch with clear migration path for multi-tenancy, multi-user, and multi-currency features.*
